...
export function render() {
  ...
  return `
    <div class="container mt-2" data-tool="${TOOL_KEY}">
        ...
        <div class="card-header py-2">
          <div class="d-flex align-items-center">
            ...
            <div style="width:90px;" class="text-end">
              <button class="btn btn-secondary btn-sm" type="button" data-role="delete_all_history">全刪</button>
            </div>
          </div>
        </div>
            ...
            <table class="table table-sm align-middle mb-0">
              <thead>
                <tr>
                  ...
                  <th style="white-space:nowrap;">TPN[AA]</th>
      ...
      <!-- Bootstrap Modal: restore/delete confirmation -->
      <div class="modal fade" tabindex="-1" aria-hidden="true" data-role="history_modal">
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" data-role="modal_title">確認</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              <div class="text-muted small mb-2" data-role="modal_subtitle"></div>
              <div class="border rounded p-2" style="background: transparent;" data-role="modal_preview"></div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-light" data-bs-dismiss="modal">取消</button>
              <button type="button" class="btn btn-secondary" data-role="modal_confirm">確定</button>
            </div>
          </div>
        </div>
      </div>
...
export function init(root) {
  ...
  const els = {
    ...
    modalEl: $('[data-role="history_modal"]'),
    modalTitle: $('[data-role="modal_title"]'),
    modalSubtitle: $('[data-role="modal_subtitle"]'),
    modalPreview: $('[data-role="modal_preview"]'),
    modalConfirm: $('[data-role="modal_confirm"]'),
  };
  ...
  function serializeState() {
    ...
    for (const el of nodes) {
      if (el.closest('[name="fluid_other_io_row"]') || el.closest('[name="fluid_other_solution_row"]')) continue;
      const key = el.dataset.role || el.getAttribute("name");
      if (!key) continue;

      if (el.type === "checkbox" || el.type === "radio") state.fields[key] = !!el.checked;
    ...
    const solRows = Array.from(box.querySelectorAll('[name="fluid_other_solution_row"]'));
    for (const r of solRows) {
      const base = r.querySelector('[data-role="other_solution_base"]')?.value ?? "";
      const rate = r.querySelector('[data-role="other_solution_rate"]')?.value ?? "";
  ...
  function snapshotSummary(snap) {
    ...
    const tpnAA = getField(snap, "tpn_aa");
    const tpnGrams = getField(snap, "tpn_grams");
    const tpnLabel = fluidTpnAAMap[tpnAA] || "TPN";
    const tpnStr = tpnGrams ? `${tpnLabel} ${tpnGrams}` : "";

    const lipidType = getField(snap, "lipid_type") || "";
    const lipidGrams = getField(snap, "lipid_grams");
    const lipidStr = (lipidType && lipidGrams) ? `${lipidType} ${lipidGrams}` : "";
  ...
  function addSnapshot(source = "manual") {
    ...
    list.unshift(snap);
    const clipped = list.slice(0, MAX_SNAPSHOTS);
    saveSnapshots(clipped);
  ...
  // ============================================================
  // Bootstrap modal controller (restore/delete)
  // ============================================================
  ...
  const openModal = ({ type, id }) => {
    ...
    const hasBootstrapModal = typeof window.bootstrap?.Modal === "function" && els.modalEl;

    const previewLines = (() => {
      if (!snap) return ["(找不到此紀錄)"];
    ...
    const previewHtml = previewLines
      .map(x => `<div style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${String(x)}</div>`)
      .join("");

    if (!hasBootstrapModal) {
      const ok = window.confirm(previewLines.join("\n") + `\n\n${type === "restore" ? "確定要回復？" : "確定要刪除？"}`);
      if (!ok) return;

      if (type === "restore" && snap) hydrateState(snap.state);
      if (type === "delete") deleteSnapshotById(id);
      if (type === "delete_all") deleteAllSnapshots();
      return;
    }
    ...
    if (els.modalSubtitle) {
      els.modalSubtitle.textContent =
        type === "restore" ? "將以此快照覆蓋目前畫面內容。" :
        type === "delete" ? "此操作無法復原。" :
        "將清空所有歷史紀錄，此操作無法復原。";
    }

    if (els.modalPreview) {
      els.modalPreview.innerHTML = previewHtml;
    }

    if (els.modalConfirm) {
      els.modalConfirm.textContent =
        type === "restore" ? "回復" :
        type === "delete" ? "刪除" :
        "全部刪除";
    }

    modalInstance = modalInstance || new window.bootstrap.Modal(els.modalEl);
    modalInstance.show();
  };
  ...
  els.modalConfirm?.addEventListener("click", () => {
    ...
    const snaps = loadSnapshots();
    const snap = snaps.find(s => s.id === pendingAction.id);

    if (pendingAction.type === "restore") {
      if (snap) hydrateState(snap.state);
    } else if (pendingAction.type === "delete") {
      deleteSnapshotById(pendingAction.id);
    } else if (pendingAction.type === "delete_all") {
      deleteAllSnapshots();
    }
  ...
  box.addEventListener("click", (e) => {
    ...
    if (restoreBtn) {
      ...
      if (id) openModal({ type: "restore", id });
      return;
    }
    ...
    if (delBtn) {
      ...
      if (id) openModal({ type: "delete", id });
      return;
    }

    // click-to-copy + auto snapshot
    const copyNode = t?.closest?.(".copy-item");
    if (copyNode) {
